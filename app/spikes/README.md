...
what graduates into the app is the *shape* (the `CredentialStore` interface,
the four-seam tool surface in `src/lore-tools.ts`), not this code.
...
# desktop tool surface (lore_cli allowlist, scoped read, ask_user, and
# load_skill for the skills' handoffs to each other), in a terminal REPL
...
2. Does the skill complete with only the four seams — including the
   cross-skill handoffs (capture → publish, onboard → enable-payments,
   publish ↔ enable-payments) via `load_skill`? Every step that doesn't is a
   skill bug to log (and fix in the skill), not a reason to widen the
   surface. Also: does capture through this feel *better* than capture
   through Claude Code — the APP-003 checkpoint gate.
