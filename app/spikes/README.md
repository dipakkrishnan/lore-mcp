...
the three-seam tool surface in `src/lore-tools.ts`), not this code.
...
# desktop tool surface (lore_cli allowlist, scoped read, ask_user), in a
# terminal REPL against a temp Lore home (~/.lore-spike-home).
...
2. Does the skill complete with only the three seams? Every step that
   doesn't is a skill bug to log (and fix in the skill), not a reason to
   widen the surface. Also: does capture through this feel *better* than
   capture through Claude Code — the APP-003 checkpoint gate.
