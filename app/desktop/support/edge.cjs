  ...
  window.webContents.once("did-finish-load", async () => {
    try {
      ...
      if (scenario === "provision") {
        ...
        await js(`window.__lore.preview({ type: "question", id: "preview-q", task: null, questions: [{ question: "What should a publication cost?", header: "Price", multiSelect: false, options: [{ label: "$0.05", description: "Higher" }, { label: "$0.01 (Recommended)", description: "Low first price" }] }, { question: "Paste your payout address.", header: "Payout", multiSelect: false, options: [] }] })`);
        check("the recommended option starts selected and is chipped", await js(`document.querySelector("#request input:checked")?.value`) === "$0.01 (Recommended)" && await js(`document.querySelector("#request .choice:has(input:checked)").textContent`) === "$0.01RecommendedLow first price");
        ...
        await sleep(200);
        check("the echo drops the marker and shortens the address", await js(`document.querySelector("#log").textContent.endsWith("$0.01 · 0x0c27…8166")`));
