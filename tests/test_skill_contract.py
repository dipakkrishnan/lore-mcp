...
# Ways a payment secret could reach a transcript: named on a command line, assigned in
# a snippet the owner is told to run, or pasted as a literal key.
SECRET_LEAKS = (
    # Horizontal whitespace only: the argument has to be on the same line to be one.
    (
        re.compile(r"lore[ \t]+payment[ \t]+auth[ \t]+(?!--)\S"),
        "passes a value to `lore payment auth`",
    ),
    (re.compile(r"CDP_API_KEY_SECRET\s*="), "assigns the CDP key secret"),
    (re.compile(r"LORE_TEST_BUYER_KEY\s*="), "assigns the test buyer key"),
...
class SkillContractTest(unittest.TestCase):
    ...
    def test_the_payment_skill_refuses_the_secret_out_loud(self) -> None:
        ...
        self.assertIn("lore payment auth", skill)
