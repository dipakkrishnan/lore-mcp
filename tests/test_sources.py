...
from lore.sources import Source, State, available_sources, scan
...
def owned(store: Store) -> list[dict[str, object]]:
    return [entry for entry in sources_module.entries(store) if entry["owned"]]
...
class OwnerFolderTest(LoreTestCase):
    ...
    def test_a_folder_is_read_as_private_memories_the_moment_it_is_added(self) -> None:
        ...
        with Store() as store:
            entry = sources_module.add(store, str(root))
    ...
    def test_the_four_states_that_are_not_connected(self) -> None:
        ...
        with Store() as store:
            ...
            states = {
                entry["name"]: entry["state"] for entry in sources_module.entries(store)
            }
    ...
    def test_adding_the_same_folder_twice_returns_the_same_source(self) -> None:
        ...
        with Store() as store:
            first = sources_module.add(store, str(root))
            again = sources_module.add(store, str(root) + "/", label="Second")
    ...
    def test_since_keeps_undated_and_recent_items_only(self) -> None:
        ...
        with Store() as store:
            entry = sources_module.add(store, str(root), since="2025-01-01")
    ...
    def test_an_unreadable_day_is_rejected_before_anything_is_stored(self) -> None:
        ...
        with Store() as store:
            with self.assertRaises(sources_module.SourceError):
                sources_module.add(store, str(root), since="last tuesday")
            with self.assertRaises(sources_module.SourceError):
                sources_module.add(store, str(root / "nowhere"))
            self.assertEqual(owned(store), [])

    def test_remove_keep_leaves_the_memories_behind(self) -> None:
        root = self.vault(note_md="# Note\n\nA note long enough to be a memory here.")
        with Store() as store:
            name = str(sources_module.add(store, str(root))["name"])
            self.assertEqual(
                sources_module.remove(store, name, delete=False),
    ...
    def test_remove_delete_keeps_only_what_a_publication_cites(self) -> None:
        ...
        with Store() as store:
            ...
            self.assertEqual(
                sources_module.remove(store, name, delete=True)["memories"],
                {"deleted": 1, "kept": 1},
            )
    ...
    def test_a_built_in_source_can_never_be_removed(self) -> None:
        with Store() as store:
            with self.assertRaises(sources_module.SourceError):
                sources_module.remove(store, "codex", delete=False)
            with self.assertRaises(sources_module.SourceError):
                sources_module.remove(store, "folder-deadbeef", delete=False)
    ...
    def test_vault_plumbing_is_never_imported(self) -> None:
        ...
        with Store() as store:
            self.assertEqual(sources_module.add(store, str(root))["imported"], 1)
    ...
    def test_reading_an_owner_folder_leaves_the_agent_sources_alone(self) -> None:
        ...
        with Store() as store:
            store.set_setting("sources", ["codex"])
            name = str(sources_module.add(store, str(root))["name"])
            reads = sources_module.read(store)
            ...
            with self.assertRaises(sources_module.SourceError):
                sources_module.read(store, ["nope"])
