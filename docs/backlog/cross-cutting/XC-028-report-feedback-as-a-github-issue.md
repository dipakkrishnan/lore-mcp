...
- [ ] `lore/feedback.py` exists with `build()`, `submit()`, and
      `report_feedback()`, fully covered per `tests/gate.py`'s 90%
      statement/branch floor.
- [ ] `feedback-relay/` deploys independently of `lore node deploy` and is
      never packaged into the `lore-mcp` wheel.
- [ ] A submitted report becomes a labeled GitHub issue on
      `dipakkrishnan/lore-mcp`, with a metadata table and no secrets in the
      response.
- [ ] `PRIVACY.md` accurately describes what this feature sends and when.
- [ ] `tests/gate.py --require-node` covers both `lore/node/` and
      `feedback-relay/`.
