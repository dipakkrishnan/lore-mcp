...
- [ ] `marketplace.json` validates against `schema.json` and the validator
      fails on a duplicate node, a non-https URL, a malformed network id, or
      a missing field.
- [ ] The workflow runs the validator with `--live` on pull requests.
- [ ] The first entry is the live node at lore.dipakrkrishnan.workers.dev
      and an agent with a fetch tool can read the raw file and reach that
      node's `discover` from it.
- [ ] README says what the file is, how to list and delist, and how an
